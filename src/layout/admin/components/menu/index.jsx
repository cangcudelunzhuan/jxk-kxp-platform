...
const Menu = function () {
  ...
  const MenuLink = ({ data, className, icon, kinship }) => (data.url ? (
    <NavLink to={data.url} className={className} activeClassName={styles.active} onClick={() => onMenuClick(kinship)}>
      {icon}
      {data.name}
      {data.children && data.children.length ? <Icon type="jiantou-copy-copy" size="small" className={styles.menu_status_icon} /> : null}
    </NavLink>
  ) : (
